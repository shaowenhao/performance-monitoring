...
public class JdbcDatabaseUtil {
	...
	public static Connection getConnection(String file) {
		...
		try {
			// System.out.println(String.format(" SQL connection config %s %s %s %s",
			...
			// System.out.println(String.format("%s", connection == null ? "null":
			// connection.toString()));
			// 返回数据库连接
			return connection;
	...
	public static Properties getConnectionProperties(String file) throws IOException {
		Properties properties = new Properties();
		InputStream input = JdbcDatabaseUtil.class.getClassLoader().getResourceAsStream("db_properties/" + file);
		properties.load(input);
