...
public class CommonCheckFunctions {
	...
	public static void checkDataContainsSpecifiedFields(String jsonPath, String fields,
	...
	public static void checkDataOnlyContainsSpecifiedFields(String jsonPath, String fields,
			List<HashMap<String, String>> responseData) {
		if (responseData == null) {
			System.out.println("Error: null is returned when try to get data from jasonPath '" + jsonPath + "'");
		}
	...
	public static void checkDataContainsNumOfItems(String jsonPath, String NumOfItems,
			List<HashMap<String, String>> responseData) {
		if (responseData == null) {
			System.out.println("Error: null is returned when try to get data from jasonPath '" + jsonPath + "'");
			return;
	...
	public static boolean isValidDateStr(String input) {
		...
		if (!input.contains("T")) {
			...
			if (input.length() < 5)
				return false;
			if (!isIntegerStr(input.substring(0, 4)))
	...
	public static boolean compareOrderFieldValue(String valueShouldBeSmall, String valueShouldBeLarge) {
		...
		{
			if (Integer.parseInt(valueShouldBeSmall) > Integer.parseInt(valueShouldBeLarge))
				return false;
		} else if ((isFloatStr(valueShouldBeSmall)) && (isFloatStr(valueShouldBeLarge))) // Compare float values
		{
			if (Float.parseFloat(valueShouldBeSmall) > Float.parseFloat(valueShouldBeLarge))
				return false;
		} else // Compare string in alphabetical order
		{
			if (valueShouldBeLarge.compareTo(valueShouldBeSmall) < 0)
				return false;
		}
	...
	public static boolean isMapSortedByKey(List<HashMap<String, String>> listOfHashMaps, String key, String order) {
		if (listOfHashMaps.isEmpty() || listOfHashMaps.size() == 1)
			return true;
		...
		while (iter.hasNext()) {
			...
			if (!compareOrderFieldValue(valueShouldBeSmall, valueShouldBeLarge))
				return false;
	...
	public static boolean ifDataSatisfiesCondition(String jasonPath, String fieldToCompare, String compareType,
			String valueToCompare, List<HashMap<String, String>> dataList) {
		if (dataList == null)
			return false;
		if (dataList.isEmpty())
			return false;
		...
		while (iter.hasNext()) {
			...
			if (currentEntry.containsKey(fieldToCompare)) {
				...
				if (checkCondition(currentFieldValue, compareType, compareValue) == false) {
					System.out.println("condition check failed: value '" + currentFieldValue
							+ "' does not satisfy condition '" + compareType + ": " + valueToCompare + "'");
					return false;
				}
			}
		}
	...
	public static boolean ifDataSatisfiesJointCondition(String fieldToCompare1, String compareType1,
			String valueToCompare1, String fieldToCompare2, String compareType2, String valueToCompare2,
		...
		while (iter.hasNext()) {
			...
			if ((checkCondition(currentFieldValue1, compareType1, compareValue1) == false)
				...
				System.out.println(
						"Joint condition check failed: value '" + currentFieldValue1 + "' does not satisfy condition '"
								+ compareType1 + ": " + valueToCompare1 + "'; and" + "value '" + currentFieldValue2
								+ "' does not satisfy condition '" + compareType2 + ": " + valueToCompare2 + "'");
	...
	public static boolean checkCondition(String valueStr, String compareType, String valueToCompare) {
		...
		try {
			switch (compareType) // Reference:
									// https://hasura.io/docs/1.0/graphql/core/queries/query-filters.html#comparision-operators
			{
			case "eq":
			...
			case "gte":
				result = ifFieldValueGreaterThan(valueStr, valueToCompare)
						|| ifFieldValueEqualsTo(valueStr, valueToCompare);
				break;
	...
	public static boolean ifFieldValueGreaterThan(String valueStr, String valueToCompare) throws ParseException {
		...
		{
			if (Integer.parseInt(valueToCompare) >= Integer.parseInt(valueStr))
				return false;
		} else if (isFloatStr(valueToCompare)) // Compare float values
		{
			if (Float.parseFloat(valueToCompare) >= Float.parseFloat(valueStr))
				return false;
		} else // Compare string
		{
			if (isValidDateStr(valueToCompare)) {
				...
				if (valueStr.compareTo(valueToCompare) <= 0)
					return false;
			}
		}
	...
	public static boolean ifFieldValueLessThan(String valueStr, String valueToCompare) throws ParseException {
		if (isIntegerStr(valueToCompare)) // Compare integer values
		{
			if (Integer.parseInt(valueStr) >= Integer.parseInt(valueToCompare))
				return false;
		} else if (isFloatStr(valueToCompare)) // Compare float values
		{
			if (Float.parseFloat(valueStr) >= Float.parseFloat(valueToCompare))
				return false;
		} else // Compare string
		{
			if (isValidDateStr(valueToCompare)) {
				...
				if (valueToCompare.compareTo(valueStr) <= 0)
					return false;
			}
		}
