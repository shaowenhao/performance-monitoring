...
from automation.core.components.ui import browser, execution, utils
from automation.core.components.ui.settings import settings
...
def _capture_screenshot(image_name):
    ...
    if not execution.report_directory:
        logger.log_debug('cannot take screenshot, report directory does not exist')
        return None
    ...
    if not settings['screenshots']:
        ...
        screenshot_settings = settings['screenshots']
        ...
        screenshot_filename = utils.save_screenshot(execution.report_directory,
...
def _run_wait_hook():
    ...
    if wait_hook:
        ...
        wait_hook_function()
        logger.log_debug('Wait hook waited for {} seconds'
...
def _screenshot_on_error():
    ...
    Use the last step message as the screenshot filename.
    """
    _screenshot_on_condition(settings['screenshot_on_error'])
...
def _screenshot_on_step():
    ...
    Use the last step message as the screenshot filename.
    """
    _screenshot_on_condition(settings['screenshot_on_step'])
...
def add_cookie(cookie_dict):
    ...
    * If the browser did not visit any URL (initial blank page) this
    ...
    cookie_dict : value
    """
    logger.log_info('Add cookie: {}'.format(cookie_dict))
...
def assert_contains(element, value):
    ...
    logger.log_warning('Action assert_contains is deprecated')
    _add_step(step_message)
    _run_wait_hook()
...
def assert_equals(actual_value, expected_value):
    ...
    logger.log_warning('Action assert_equals is deprecated')
    _add_step(step_message)
...
def assert_false(condition):
    ...
    logger.log_warning('Action assert_false is deprecated')
    _add_step(step_message)
...
def assert_true(condition):
    ...
    logger.log_warning('Action assert_true is deprecated')
    _add_step(step_message)
...
def capture(message=''):
    ...
    """
    logger.log_warning('capture is DEPRECATED, Use take_screenshot')
...
def clear(element):
    ...
    element : element
    """
    logger.log_warning('clear is DEPRECATED, use clear_element')
...
def close():
    ...
    logger.log_warning('close is DEPRECATED, use close_browser or close_window')
    close_browser()


def close_browser():
    """Close browser and all it's windows/tabs"""
    logger.log_info('Close browser')
    get_browser().quit()
...
def debug():
    ...
    Enter debug mode"""
    logger.log_warning('debug is DEPRECATED, use interactive_mode')
    interactive_mode()
...
def get_active_element():
    ...
    logger.log_debug('Get active element')
    return get_browser().switch_to.active_element


def get_alert_text():
    """Get alert text"""
    logger.log_debug('get alert text')
...
def get_cookie(name):
    ...
    name : value
    """
    logger.log_debug('Get cookie "{}"'.format(name))
...
def get_cookies():
    ...
    """
    logger.log_debug('Get all cookies')
...
def get_element_attribute(element, attribute):
    ...
    """
    element = get_browser().find(element)
    logger.log_debug("Get '{}' element '{}' attribute".format(element, attribute))
...
def get_element_text(element):
    ...
    """
    element = get_browser().find(element)
    logger.log_debug("Get '{}' element text".format(element))
    return element.text
...
def get_window_index():
    ...
    logger.log_debug('Get current window index')
...
def interactive_mode():
    ...
    if not settings['interactive']:
        logger.log_info('the -i flag is required to access interactive mode')
        return
    try:
...
def maximize_window():
    ...
    logger.log_debug('maximize browser window')
...
def mouse_hover(element):
    ...
    element : element
    """
    logger.log_warning('mouse_hover is DEPRECATED, use mouse_over instead')
...
def random(value):
    ...
    for char in value:
        if char == 'c':
            ...
            random_string += char
    logger.log_info('Random value generated: {}'.format(random_string))
...
def select_by_index(element, index):
    ...
    index : value
    """
    logger.log_warning('select_by_index is DEPRECATED, use select_option_by_index')
...
def select_by_text(element, text):
    ...
    text : value
    """
    logger.log_warning('select_by_text is DEPRECATED, use select_option_by_text')
...
def select_by_value(element, value):
    ...
    value : value
    """
    logger.log_warning('select_by_value is DEPRECATED, use select_option_by_value')
...
def set_trace():
    ...
    """
    if not settings['interactive']:
        logger.log_info('the -i flag is required to set_trace')
        return
    pdb.set_trace()
...
def store(key, value):
    ...
    value : value
    """
    logger.log_info("Store value '{}' in key '{}'".format(value, key))
...
def store_secret(key, value):
    ...
    value : value
    """
    logger.log_info("Stored a key and value into secrets")
...
def timer_start(timer_name=''):
    ...
    if timer_name in execution.timers:
        logger.log_debug('timer "{}" is already been started'.format(timer_name))
    else:
...
def timer_stop(timer_name=''):
    ...
    if timer_name in execution.timers:
        ...
        logger.log_debug('timer {} stopped: {}'.format(timer_name, elapsed_time))
    else:
        logger.log_debug('timer {} has not been started'.format(timer_name))
    return elapsed_time
...
def verify_alert_is_not_present():
    ...
    logger.log_warning('verify_alert_is_not_present is DEPRECATED, use verify_alert_not_present')
    verify_alert_not_present()


def verify_alert_is_present():
    """DEPRECATED, use verify_alert_present"""
    logger.log_warning('verify_alert_is_present is DEPRECATED, use verify_alert_present')
...
def verify_cookie_exists(name):
    ...
    name: value
    """
    logger.log_warning('verify_cookie_exists is DEPRECATED, use verify_cookie_present')
...
def verify_exists(element):
    ...
    element : element
    """
    logger.log_warning('verify_exists is DEPRECATED, use verify_element_present')
...
def verify_is_enabled(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_enabled is DEPRECATED, use verify_element_enabled')
...
def verify_is_not_enabled(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_not_enabled is DEPRECATED, use verify_element_not_enabled')
...
def verify_is_not_selected(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_not_selected is DEPRECATED, use verify_element_not_checked')
...
def verify_is_not_visible(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_not_visible is DEPRECATED, use verify_element_not_displayed')
...
def verify_is_selected(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_selected is DEPRECATED, use verify_element_checked')
...
def verify_is_visible(element):
    ...
    element : element
    """
    logger.log_warning('verify_is_visible is DEPRECATED, use verify_element_displayed')
...
def verify_not_exists(element):
    ...
    element : element
    """
    logger.log_warning('verify_not_exists is DEPRECATED, use verify_element_not_present')
...
def verify_selected_option(element, text):
    ...
    text : value
    """
    logger.log_warning(('verify_selected_option is DEPRECATED, use '
...
def verify_text(text):
    ...
    text : value
    """
    logger.log_warning('verify_text is DEPRECATED, use verify_page_contains_text')
...
def verify_text_in_element(element, text):
    ...
    """
    logger.log_warning(
        'verify_text_in_element is DEPRECATED, use verify_element_text or verify_element_text_contains')
...
def wait(seconds):
    ...
    """
    logger.log_info('Waiting for {} seconds'.format(seconds))
    try:
...
def wait_for_element_not_exist(element, timeout=20):
    ...
    """
    logger.log_warning('wait_for_element_not_exists is DEPRECATED, use wait_for_element_not_present')
...
def wait_for_element_not_visible(element, timeout=20):
    ...
    timeout (optional, default: 20) : value
    """
    logger.log_warning('wait_for_element_not_visible is DEPRECATED, use wait_for_element_not_displayed')
...
def wait_for_element_visible(element, timeout=20):
    ...
    timeout (optional, default: 20) : value
    """
    logger.log_warning('wait_for_element_visible is DEPRECATED, use wait_for_element_displayed')
