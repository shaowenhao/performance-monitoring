...
from automation.core.exceptions import (IncorrectSelectorType,
                                        ElementNotFound,
                                        ElementNotDisplayed)
...
def _find_webelement(root, selector_type, selector_value, element_name,
    ...
    while webelement is None:
        try:
            ...
            logger.log_debug('Element found')
        except:
            if remaining_time() <= 0:
                break
            else:
                time.sleep(0.5)
                logger.log_debug('Element not found yet, remaining time: {:.2f}'
    ...
    if webelement is None:
        ...
        if wait_displayed:
            while not webelement.is_displayed() and remaining_time() > 0:
                logger.log_debug('Element still not visible, waiting')
                time.sleep(0.5)
...
def _find(self, element=None, id=None, name=None,
    ...
    from automation.core.components.ui.webdriver.extended_webelement import extend_webelement, ExtendedWebElement
    webelement = None
...
def _find_all(self, element=None, id=None, name=None, link_text=None,
    ...
    """
    # TODO: avoid circular import
    from automation.core.components.ui.webdriver.extended_webelement import extend_webelement
    webelements = []
